...
v_earth = np.sqrt(G * M_sun / a_earth)

# 初始状态向量 [x, y, vx, vy] * 2
y0 = [
    a_jupiter, 0.0, 0.0, v_jupiter,  # 木星
    a_earth, 0.0, 0.0, v_earth       # 地球
]
...
for res in resonances:
    ...
    plt.text(sol.t[-1], res + 0.02, f'{int(res)}:{int(1)} 共振' if res.is_integer() else f'{int(res*resonances[0])}:{int(resonances[0])} 共振', fontsize=10, color='r', ha='right')
